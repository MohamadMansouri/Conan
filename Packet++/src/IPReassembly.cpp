...
{
//	LoggerPP::setAllModlesToLogLevel(Debug); 	
...
//		}
	...
	if (gotLastFragment)
	{
		...
		//if (tempPacket->getLayerOfType<IPv4Layer>()->getIPv4Header()->protocol == PACKETPP_IPPROTO_TCP)
		//{
	//	}
		...
		//reassembledPacket->printProtocolTypes(); 
		...
		status = REASSEMBLED;
		//IPv4Layer* payLayer = (IPv4Layer*)reassembledPacket->getLayerOfType<IPv4Layer>();
		//TcpLayer* tcpLayer = (TcpLayer*)payLayer->getNextLayer();
		
	//	payLayer->setNextLayer(tcpLayer);
	//	if (payLayer->getProtocol() == TCP )
	//	{
	//		printf("%x\n",tcpLayer);
	//	}
